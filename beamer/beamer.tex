...
\begin{document}
	...
	\subtitle{Introduction to \textbf{Static Games}\\based on Bressan, \emph{Noncooperative Differential Games. A Tutorial, 2010}}
	%\logo{}
	%\institute{}
	...
	\begin{frame}{\secname}
		...
		\begin{minipage}{.7\textwidth}
			$R^A(b) = \{ a \in A \mid \Phi^A(a, b) = \max_{\omega \in A} \Phi^(\omega, b)\}$\\
			$R^B(a) = \{ b \in B \mid \Phi^B(a, b) = \max_{\omega \in B} \Phi^(a, \omega)\}$
		\end{minipage}
	...
	\begin{frame}{\secname}
		\begin{tcolorbox}[colback=rwth-10,colframe=rwth,title=upper semi-continuous, sharp corners]
			$F: X \rightrightarrows Y$ a multifunction with compact values.
			\begin{itemize}
				\item $F$ is upper semicontinuous iff $\forall x \in X \, \forall \epsilon > 0 \, \exists \delta = \delta(\epsilon) > 0$ such that $F(x') \subseteq B_{\epsilon}(F(x))$ for all $d(x', x) < \delta$
				\item equivalently $graph(F) = \{ (x,y) \mid y \in F(x) \}$ is closed
				\item $F, G$ upper semicontinuous implies $(F \times G): (x,y) \mapsto F(x) \times G(x)$ also upper semicontinuous
			\end{itemize}
