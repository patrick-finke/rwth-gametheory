...
\begin{abstract}
    In this seminar report, we are going to give an introduction to static games in game theory. The content will be heavily inspired by section 2 of \cite{bressan2010}. However, in contrast to \cite{bressan2010}, we will try to extend definitions and statements to an arbitrary number of players, wherever it is sensible to do so.

    In section~\ref{sec:static-games} of this report, we explain the base setup, including a definition of a $N$-player static game together with some examples. In section~\ref{sec:solution-concepts} we explore two different solution concepts for static games, namely the Pareto optimum and Nash equilibrium. We will write down some basic properties and find the solutions to the example games in section~\ref{sec:static-games}. We continue our analysis of the Nash equilibrium in section~\ref{sec:existence-nash}, where we proof a fundamental result on the existence of such Nash equilibria. In section~\ref{sec:randomized-strategies}, we extend this result to the class of randomized strategies, which will allow us to drop some of the assumptions we made earlier.

    We are going to fall back to $2$-player games for the last two sections. Section~\ref{sec:zero-sum-games} will be about so called zero-sum games. We will look at the notions of saddle point and value of the game of such zero-sum games and connect them to the Nash equilibrium. At the end, in section~\ref{sec:co-co-solution}, we will take a look at the cooperative-competitive solution, which is a solution conecpt designed in such a way, that it should become appealing for two parties to cooperate.

    Additionally, on request of my supervisor, the appendix will include a brief summary of the other talks of the seminar.
\end{abstract}
...
\begin{notation}
    ...
    \end{equation*}
    we write $x_i$ to represent the $i$-th component of that vector and
    \begin{equation*}
        x_{-i} = (x_1, \dotsc, x_{i-1}, x_{i+1}, \dotsc, x_n)
    \end{equation*}
    to represent the rest. Sometimes, if for example subscripts become to crowded, we switch to superscripts $x^i$ and $x^{-i}$. These have alway priority, i.e. $(x_i)_j$ becomes $x^i_j$ and not $x^j_i$.
    ...
    \end{equation*}
    to put emphasize of the $i$-th component. This also allows us to quite easily substitute a component. Let $y \in \R^n$, then
    \begin{equation*}
...
To start with the theory of static games, our first definition should be the following~\cite{schwarz}
\begin{definition}[static game]
    A (static) game for $N \in \N$ players is described by
    \begin{enumerate}
        \item sets of strategies $X_i$
        \item payoff functions $\Phi_i: \bigtimes_{i=1}^N X_i \rightarrow \R$
    \end{enumerate}
    for each player $i=1,\dotsc,N$. We write $\Gamma = \{X_i, \Phi_i\}_{i=1}^N$ and call $\Gamma$ a $N$-player game. The goal of each player is to maximize her payoff, i.e.
    \begin{equation*}
...
To get a better understanding of definition~\ref{def:static-game} and to have some material we can apply future concepts on, we want to look at some examples. Before we proceed, note the following: Out of simplicity, we only choose examples that involve two players and a finite number of strategies. However, with the exeptions of sections~\ref{sec:zero-sum-games} and \ref{sec:co-co-solution}, the theory is by no means limited by the number of players or strategies.

The following example can also be found in~\cite{bressan2010, schwarz}.
\begin{example}[Prisoner's Dilemma]
    ...
    \end{itemize}
    This situation can be modelled as a game for 2 players in the following way: The sets of strategies for the players are $X_1 = X_2 = \{C,N\}$, where $C$ indicates confessing and $N$ not confessing. The playoff functions are as follows:
    \begin{align*}
    ...
    We can also represent this 2 player game as a bi-matrix, that is a matrix with two values in each of its fields. The bi-matrix for the prisoner's dilemma can be found in table~\ref{tab:prisonders-dilemma}.
...
Another example, that should be well known to most (and can also be found in~\cite{schwarz}), is
...
In this section we will look at two different solution concepts, namely the Pareto optimum and Nash equilibrium. For the latter, we will analyze its properties and later prove an existence theorem in section~\ref{sec:existence-nash}. We also revisit the examples of section~\ref{sec:static-games} to find their Pareto optima and Nash equilibria.

Let us first look at the Pareto optimum.
\begin{definition}[Pareto optimum]
    Consider a $N$-player game $\Gamma = \{X_i, \Phi_i\}_{i=1}^N$. We call a strategy vector $x^* \in \bigchi$ Pareto optimal iff there exists no other strategy vector $x \in \bigchi$ and player index $1 \leq i \leq N$ such that
    \begin{equation*}
...
\begin{example}[Prisoner's Dilemma]
    ...
    Let us for example check $(C, N)$. There is no pair of strategies that could (strictly) increase the payoff of player $1$. For player $2$ all other pairs of strategies strictly increase her payoff, but the payoff of player $1$ gets strictly decreased for each of them. Thus $(C, N)$ is Pareto optimal. By symmetry, this also proves that $(N, C)$ is Pareto optimal.
\end{example}
...
\begin{remark}
    ...
    In a $N$-player zero-sum game all strategy vectors are Pareto optimal. We call a game $\Gamma = \{X_i, \Phi_i\}_{i=1}^N$ zero-sum iff the sum of the payoff functions is zero, i.e.
    \begin{equation*}
    ...
    Let us continue with a short proof of the statement. Assume there exists a strategy $\widetilde{x} \in \bigchi$ that is not Pareto optimal, i.e. there exists $x \in \bigchi$ and $1 \leq i \leq N$ such that
    \begin{equation*}
...
Let us now take a look at the Nash equilibrium, a key concept for the theory of competitive games, which will accompany us through the following sections.
\begin{definition}[Nash equilibrium]
    Consider a $N$-player game $\Gamma = \{X_i, \Phi_i\}_{i=1}^N$. A strategy vector $x^{*} \in \bigchi$ is called a Nash equilibrium iff
    \begin{equation*}
...
Unlike the Pareto optimum, a Nash equilibrium models a competitive situation, as described in the following
\begin{remark}
...
Let us now find an equivalent formulation to the definition of Nash equilibrium~\cite{bressan2010, schwarz}. We need the following
...
\begin{lemma}
    Consider a $N$-player game $\Gamma = \{X_i, \Phi_i\}_{i=1}^N$. A vector $x^* \in \bigchi$ is a Nash equilibrium if and only if it is a fixed point of the best response map, i.e.
    \begin{equation*}
        x^* \in S(x^*).
    \end{equation*}
...
\begin{example}[Prisoner's Dilemma]
By checking, we find $(C,C)$ as the unique Nash equilibrium. Combining this result with the result in example~\ref{ex:pareto-optimum-prisoners-dilemma}, we see that all strategies but the Nash equilibrium are Pareto optimal, i.e. optimal from a social standpoint. This explains why the prisoner's dilemma is such a dilemma: The Nash equilibrium, which is optimal from a competitive standpoint and thus will propably be choosen by the two prisoners given their situation, is the only stratgy that is not favourable for the group.
...
Here are a few properties of Nash equilibria~\cite{bressan2010}, some of which we have already seen in the preceding examples.
\begin{lemma}
...
In this section, we continue with our analysis of Nash equilibria and prove an existence theorem. To do this, we need to place suitable assumptions on the payoff functions $\Phi_i$, i.e. suitable convexity (or in this case concavity) assumptions.
...
Let us now formulate the existence theorem, which is the $N$-player equivalent of the one found in~\cite{bressan2010}.
\begin{theorem}[Existence of Nash equilibria]
    ...
    \end{equation*}
    for all $i=1,\dotsc,N$. Then, the game $\Gamma$ admits a Nash equilibrum.
    \label{thm:existence-nash}
\end{theorem}

To prove this result, we are going to use the fixpoint theorem of Kakutani (which can be found in the appendix of \cite{bressan2010}). Recall that by lemma~\ref{lem:nash-fixed-point}, $x^* \in \bigchi$ is a Nash equilibrium if and only if it is a fixed point of the best reply map $S$. First, a quick remark on notation:
\begin{remark}
...
\begin{theorem}[Fixpoint Theorem of Kakutani]
...
\begin{proof}[Proof (of theorem~\ref{thm:existence-nash})]
    ...
    \end{alignat*}
    Thus,  $(\omega, \omega') \in \graph(S)$. This means $\graph(S)$ is closed and, by lemma~\ref{lem:uppersemicontinuous-graph-closed}, $S$ is uppersemicontinuous. The Fixpoint Theorem of Kakutani yields a fixed point $x^* \in S(x^*)$ which is, by lemma~\ref{lem:nash-fixed-point}, a Nash equilibrium.
    ...
    \begin{enumerate}
        \item Note that $S_i(x_{-i})$ is bounded as a subset of $X_i$, which itself is bounded.
            ...
            \end{equation*}
            for some $\omega \in X_i$. We show $\omega \in S_i(x_{-i})$. As for $n \in \N$, $\omega_n \in S_i(x_{-i})$ it holds that $\Phi_i(\omega_n, x_{-i}) = m_i(x_{-i})$ and, as $\Phi_i$ is continuous, $\Phi_i(\omega, x_{-i}) = m_i(x_{-i})$. Thus, $\omega \in S_i(x_{-i})$ which means $S_i(x_{-i})$ is closed.
...
\begin{definition}
    Consider a $N$-player game $\Gamma = \{X_i, \Phi_i\}_{i=1}^N$. A randomized strategie is a probability measure $\mu_i \in \P(X_i)$, where $\P(X_i)$ is the set of all probability measures on the set of strategies $X_i$. The corresponding payoff functions are defined as
    \begin{equation*}
...
With this relaxed notion of strategy, we will be able to prove the following. Again, we refer to~\cite{bressan2010} for the $2$-player equivalent.
...
\begin{proof}
    ...
    \begin{enumerate}
        \item Let us first consider the following (simple) case where each $X_i$ is finite, i.e.
            ...
            \end{equation*}
            where $\mi{k} \in \I \coloneqq \{k = (k_1, \dotsc, k_N) \in \N^N : 1 \leq k_i \leq n_i \text{ for all } i = 1,\dotsc,N\}$ is a (feasible) multiindex.
            ...
            \end{equation*}
            We see that $\widetilde{\Phi}^i$ are linear in each component, hence continuous. In particular $\widetilde{\Phi}^i$ is linear in its $i$-th component, thus also concave in its $i$-th component. We can apply theorem~\ref{thm:existence-nash} to optain the existence of a Nash equilibrium $\hat{x}^* \in \Delta_{n_1} \times \cdots \times \Delta_{n_N}$.
        ...
        \item We claim that $\mu_*$ is a Nash equilibrium solution to the original game $\widetilde{\Gamma}$. We must show that
            ...
            Let $\{\varphi^i_1, \dotsc, \varphi^i_M\}$ be a continuous partition of unity on $X_i$, subordinate to the covering $\B^i$. For further reading on paritions of unity and their properties we refer to~\cite{alt2006}.
            ...
            \begin{equation*}
                ...
                \sum_{\mi{k} \in \I} \prod_{j = 1}^N \varphi^j_{k_j}(x^j_{k_j}) = \prod_{j = 1}^N \left( \sum_{1 \leq k_j \leq n_j} \varphi^j_{k_j}(x^j_{k_j}) \right) = 1
            \end{equation*}
...
Because the following theory seems not to be available in the literature for an arbitraty number of players, we are going to restrict outselves to $2$ player games. For example, we could not find an equivalent result to the Von Neumann Minimax Theorem (see corollary~\ref{cor:zero-sum-existence-nash}).

Let us first define what a $2$-player zero-sum game is.
\begin{definition}
...
We see that in a $2$ player zero-sum game the gain of one player is anothers loss, i.e. $\Phi_1 = -\Phi_2$. In this sense, a zero-sum game can be considered purely competitive.
...
Let us now also connect saddle points with the value of the game.
\begin{theorem}
...
We interpret a positive value of $V^\flat$ as ``player $1$ is stronger than player $2$'' and a negative value as ``player $2$ is stronger than player $1$''. The magnitude of $V^\flat$ is a measure of just how much stronger this stronger player is.
...
The co-co value is what each player should earn, considering her relative strength. Note that each player gets half of the playoff of the cooperative game $\Gamma^\#$ and player 1 earns an extra $V^\flat$ (if $V^\flat$ is positive, i.e. player $1$ is stronger).
